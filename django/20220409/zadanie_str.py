# zamienić podkreślniki,żeby wszystko zadziałało
...
def main():
    ...
    # 2.
    assert "KtO To tAK napISaŁ?!"
    "KtO To tAK napISaŁ?!".lower() == 'kto to tak napisał?!'
    ...
    # 4.
    assert "uwaga:sZAzGAyDKfOWr"
    "uwaga:sZAzGAyDKfOWr"[6::3] == "szyfr"

    # 5.
    assert "Ala ma kota i psa"
    "Ala ma kota i psa".split()[2] == "kota"  # trzecie słowo

    # 6.
    assert "1,25; 3,14; 22,85"
    "1,25; 3,14; 22,85".replace(",", ".").replace(';', ',') == "1.25, 3.14, 22.85"
    ...
    # 10.
    a = [2, 1, 4, 3]
    assert a.sort() == None # funkcja działająca w miejscu (in place) 
    assert a == [1, 2, 3, 4]
